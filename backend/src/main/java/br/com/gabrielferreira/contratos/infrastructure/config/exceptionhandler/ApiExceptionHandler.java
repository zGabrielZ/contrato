package br.com.gabrielferreira.contratos.infrastructure.config.exceptionhandler;

import br.com.gabrielferreira.contratos.infrastructure.mapper.ErroPadraoMapper;
...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ErroPadraoModel> naoEncontradoException(NaoEncontradoException e, HttpServletRequest request) {
        ...
        ErroPadraoModel erroPadraoModel = erroPadraoMapper.toErroPadrao(ZonedDateTime.now(UTC), httpStatus.value(), "Não encontrado", e.getMessage(), request.getRequestURI());
    ...
    public ResponseEntity<ErroPadraoModel> naoEncontradoException(RegraDeNegocioException e, HttpServletRequest request) {
        ...
        ErroPadraoModel erroPadraoModel = erroPadraoMapper.toErroPadrao(ZonedDateTime.now(UTC), httpStatus.value(), "Regra de negócio", e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(httpStatus).body(erroPadraoModel);
    }
    ...
    public ResponseEntity<ErroPadraoModel> validacaoException(MethodArgumentNotValidException e, HttpServletRequest request) {
        HttpStatus httpStatus = HttpStatus.BAD_REQUEST;
        ErroPadraoCamposModel erroPadraoCamposModel = erroPadraoMapper.toErroPadraoCampos(ZonedDateTime.now(UTC), httpStatus.value(), "Erro validação de campos", "Ocorreu um erro de validação nos campos", request.getRequestURI());
    ...
    public ResponseEntity<ErroPadraoModel> erroException(Exception e, HttpServletRequest request) {
        ...
        ErroPadraoModel erroPadraoModel = erroPadraoMapper.toErroPadrao(ZonedDateTime.now(UTC), httpStatus.value(), "Erro inesperado", "Ocorreu um erro inesperado no sistema, tente mais tarde", request.getRequestURI());
        return ResponseEntity.status(httpStatus).body(erroPadraoModel);
    }
}
